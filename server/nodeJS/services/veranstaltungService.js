...
async function getVeranstaltungen(limit = 25, istGenehmigt = 1, bis, userId = 0){
    ...
    if(userId !== 0){
        results = (await conn.query(queryWithUserFavorites, [Number(userId), Number(istGenehmigt), bis, Number(limit)]).catch(error => {console.log(error); return { error: "Fehler bei Db" };}))
    }
    else{
        results = (await conn.query(query, [Number(istGenehmigt), bis, Number(limit)]).catch(error => {console.log(error); return { error: "Fehler bei Db" };}))
    }
